...
export async function exportPortfolioReport(
  ...
  doc.setFontSize(14);
  doc.text("RAIDASH PORTFOLIO REPORT", 12, 11.5);
  ...
  if (!chartLoaded) {
    ...
    doc.setFontSize(9);
    doc.text(
      "Net Worth Trend Graph (Interactive visualization on your Raidash web dashboard)",
      pageWidth / 2,
      99.5,
      { align: "center" }
    );
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    "Generated by Raidash - Privacy-first wealth tracking. Personal report for CA / Financial Advisor review. Not financial advice.",
    pageWidth / 2,
    286.5,
    { align: "center" }
  );
  ...
  doc.save(`raidash-report-${session.metadata.statement_timestamp.replace(/[^0-9a-zA-Z-]/g, "_")}.pdf`);
}
